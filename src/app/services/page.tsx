    ...
    <>
      <section className="bg-[#1B3A5C] text-white py-20">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <p className="text-[#a0b4c8] text-xl max-w-xl">No bloat. No enterprise software. Just fixes that work for the way your business actually runs.</p>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-6">
          {services.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {services.map((s) => (
                <Link key={s._id} href={`/services/${s.slug.current}`} className="border border-[#E5E7EB] rounded-lg p-8 hover:border-[#F5C518] hover:shadow-md transition-all group">
                  ...
                  {s.tagline && <p className="text-[#6B7280] text-sm leading-relaxed mb-4">{s.tagline}</p>}
                  <span className="text-[#1B3A5C] text-sm font-bold group-hover:text-[#F5C518] transition-colors">Learn more →</span>
                </Link>
              ))}
            </div>
          ) : (
            <p className="text-[#6B7280]">Services coming soon.</p>
          )}
          ...
          <div className="mt-16 bg-[#F8F8F8] rounded-lg p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            ...
            <Link href="/contact" className="bg-[#F5C518] text-[#1B3A5C] font-bold px-8 py-3 rounded hover:bg-[#D4A800] transition-colors whitespace-nowrap">
              Get a Free Audit
            </Link>
          </div>
        </div>
